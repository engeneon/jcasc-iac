     ...
     CURRENT_BRANCH="main"
      ...
      stage("Getting infrastructure code") {
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-0-test/") {
                deleteDir()
              }
        ...
        stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/level-0-test/orion/configuration/") {
        ...
        stage("Configuring Azure Access") {
            steps {
              dir("/var/jenkins_home/workspace/level-0-test/orion/level-0") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Deploying Level-0 Infrastructure") {
         steps {
           ...
           dir("/var/jenkins_home/workspace/level-0-test/orion/level-0/storage") {  
                sh '''#!/bin/bash
                set -ex
                echo "Deploying foundation state storage for terraform ..."
                pwd
                ls -l
                ./test.sh
                '''           
           ...
           dir("/var/jenkins_home/workspace/level-0-test/orion/level-0/image-gallery") {
                sh '''#!/bin/bash
                set -ex
                echo "Deploying bootstrap o.s image gallery ..."
                pwd
                ls -l
                ./test.sh
                '''
           ...
           dir("/var/jenkins_home/workspace/level-0-test/orion/level-0/keyvaults") {
                sh '''#!/bin/bash
                set -ex
                echo "Deploying foundation keyvaults for deployment secrets ..."
                pwd
                ls -l
                ./test.sh
                '''
           ...
           dir("/var/jenkins_home/workspace/level-0-test/orion/level-0/secrets") {
                sh '''#!/bin/bash
                set -ex
