   ...
   CURRENT_BRANCH="main"
      ...
      stage("Getting infrastructure code") {
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-3-test/") {
                deleteDir()
              }
      ...
      stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/level-3-test/orion/configuration/") {
       ...
       stage("Configuring Azure Access") {
            steps {
             dir("/var/jenkins_home/workspace/level-3-test/orion/level-3") {
               sh '''#!/bin/bash
               set -ex
        ...
        stage("Deploying Level-3 Infrastructure") {
         steps {
           dir("/var/jenkins_home/workspace/level-3-test/orion/level-3/dns-zone-records") {  
                sh '''#!/bin/bash
                set -ex
                pwd
                ls -l
