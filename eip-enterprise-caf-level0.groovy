    ...
    stages {
      ...
      stage("Getting infrastructure code") {
        ...
        stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/level-0/orion/configuration/") {
        ...
        stage("Configuring Azure Access") {
            steps {
              dir("/var/jenkins_home/workspace/level-0/orion/level-0") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Deploying Level-0 Infrastructure") {
         steps {
           ...
           dir("/var/jenkins_home/workspace/level-0/orion/level-0/storage") {  
                sh '''#!/bin/bash
                set -ex
           ...
           dir("/var/jenkins_home/workspace/level-0/orion/level-0/image-gallery") {
                sh '''#!/bin/bash
                set -ex
           ...
           dir("/var/jenkins_home/workspace/level-0/orion/level-0/keyvaults") {
                sh '''#!/bin/bash
                set -ex
