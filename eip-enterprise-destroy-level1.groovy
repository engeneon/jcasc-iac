   ...
   stages {
    ...
    stage("Getting infrastructure code") {
            steps {
              ...
              dir("/var/jenkins_home/workspace/destroy-level-1/") {
                deleteDir()
              }
        ...
        stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/destroy-level-1/orion/configuration/") {
        ...
        stage("Configuring Azure Access") {
            steps {
              dir("/var/jenkins_home/workspace/destroy-level-1/orion/level-1") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Destroying Level-1 Infrastructure") {
         steps {

           dir("/var/jenkins_home/workspace/destroy-level-1/orion/level-1/azure-caf-implementation") {
                sh '''#!/bin/bash
           ...
           dir("/var/jenkins_home/workspace/destroy-level-1/orion/level-1/azure-caf-implementation") {
                sh '''#!/bin/bash
                echo "Destroying Azure CAF Management and Networking Foundations ..."
                pwd
                ls -l
                ./destroy.sh
                '''
