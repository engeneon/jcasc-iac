    ...
    stages {
      ...
      stage("Getting infrastructure code") {
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-1-test/") {
                deleteDir()
              }
        ...
        stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/level-1-test/orion/configuration/") {
        ...
        stage("Configuring Azure Access") {
            steps {
             dir("/var/jenkins_home/workspace/level-1-test/orion/level-1") {
               sh '''#!/bin/bash
               set -ex
        ...
        stage("Deploying Level-1 CAF Infrastructure") {
         steps {
           ...
           dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/alerting") {
                sh '''#!/bin/bash
                set -ex
                echo "Testing Security Alerting Framework ..."
                pwd
                ls -l
                ./test.sh
                '''
           ...
           dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/azure-caf-implementation") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Deploy Expressroute infrastructure") {
          steps {
           dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/expressroute") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Deploy Private DNS Forwarding infrastructure") {
          steps {
             dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/dns-private") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Configure IP address reservations") {
          steps {
             dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/ip-reservations") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("GitOps infrastructure") {
          steps {
             ...
             dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/git-ops") {
                sh '''#!/bin/bash
                set -ex
        ...
        stage("Azure Bastion Service") {
          steps {
             ...
             dir("/var/jenkins_home/workspace/level-1-test/orion/level-1/bastion-service") {
                sh '''#!/bin/bash
                set -ex
