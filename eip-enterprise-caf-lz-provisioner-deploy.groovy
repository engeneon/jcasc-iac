pipelineJob("landing-zone-provisioner-deploy") {
  definition {
    cps {
         sandbox(true)
script("""
      ...
      stages {
        ...
        stage("Configuring Azure Access") {
            ...
            steps {
              ...
              dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy") {
                deleteDir()
              }
        ...
        stage("Processing Landing Zone Configuration") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/configuration") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
              #./config.sh eip-dev-ea-lz000
              echo "============================================="
              '''
            }
          }
        ...
        stage("Deploying RBAC Roles for Landing Zone") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-0/aad-rbac") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        stage("Deploying Landing Zone Automation Credentials") {
          steps {
            ...
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-0/secrets") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
            ...
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-0/keyvaults") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
            ...
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-0/storage") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        //BEGIN process configuration
        stage("Deploying Landing Zone Spoke 000 VNETs") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-2/caf-spokes/dev-ea-lz000-spoke-000") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        stage("Configuring Central Firewall Access") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-2/afw-policy") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        stage("Configuring Landing Zone Hub-Spoke Peering Link") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-1/azure-caf-implementation") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        stage("Customising Landing Zone Policy") {
          steps {
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-3/custom-landing-zones") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
        ...
        stage("Deploying Landing Zone DNS Configuration") {
          steps {
            ...
            dir("/var/jenkins_home/workspace/landing-zone-provisioner-deploy/orion/level-3/dns-zone-records") {  
              sh '''#!/bin/bash
              set +ex
              ls -l
              pwd
