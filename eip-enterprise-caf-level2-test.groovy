   ...
   CURRENT_BRANCH="main"
      ...
      stage("Getting infrastructure code") {
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-2-test/") {
                deleteDir()
              }
        ...
        stage("Extracting global environment variables") {
         steps {
           dir("/var/jenkins_home/workspace/level-2-test/orion/configuration/") {
        ...
        stage("Configuring Azure Access") {
          ...
          steps {
             dir("/var/jenkins_home/workspace/level-2-test/orion/level-2") {
               sh '''#!/bin/bash
               set -ex
       ...
       stage("Deploying Level-2 Infrastructure") {
            ...
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-2-test/orion/level-2/afw-policy") {
        ...
        stage("Deploying Consumer Spokes") {
            ...
            steps {
              ...
              dir("/var/jenkins_home/workspace/level-2-test/orion/level-2/caf-spokes/spoke-000") {
                   sh '''#!/bin/bash
                   set -ex
              ...
              dir("/var/jenkins_home/workspace/level-2-test/orion/level-2/caf-spokes/spoke-001") {
                   sh '''#!/bin/bash
                   set -ex
              ...
              dir("/var/jenkins_home/workspace/level-2-test/orion/level-2/caf-spokes/spoke-002") {
                   sh '''#!/bin/bash
                   set -ex
